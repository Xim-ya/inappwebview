...
public class JavaScriptBridgeJS {
        public static final String JAVASCRIPT_BRIDGE_NAME = "toappHandler";
        ...
        public static final String WEB_MESSAGE_CHANNELS_VARIABLE_NAME = "window." + JAVASCRIPT_BRIDGE_NAME
                        + "._webMessageChannels";
        ...
        public static final String UTIL_JS_SOURCE = JAVASCRIPT_UTIL_VAR_NAME + " = {" +
                        ...
                        "            this._bodyText = body;" +
                        "        } else if (" + JAVASCRIPT_UTIL_VAR_NAME
                        ...
                        "        } else if (" + JAVASCRIPT_UTIL_VAR_NAME
                        + ".support.searchParams && URLSearchParams.prototype.isPrototypeOf(body)) {" +
                        "            this._bodyText = body.toString();" +
                        "        } else if (" + JAVASCRIPT_UTIL_VAR_NAME + ".support.arrayBuffer && "
                        + JAVASCRIPT_UTIL_VAR_NAME + ".support.blob && " + JAVASCRIPT_UTIL_VAR_NAME
                        + ".isDataView(body)) {" +
                        "            this._bodyArrayBuffer = bufferClone(body.buffer);" +
                        "            this._bodyInit = new Blob([this._bodyArrayBuffer]);" +
                        "        } else if (" + JAVASCRIPT_UTIL_VAR_NAME
                        + ".support.arrayBuffer && (ArrayBuffer.prototype.isPrototypeOf(body) || isArrayBufferView(body))) {"
                        +
                        ...
                        "        }" +
                        "        if (" + JAVASCRIPT_UTIL_VAR_NAME + ".isString(body) || (" + JAVASCRIPT_UTIL_VAR_NAME
                        + ".support.searchParams && body instanceof URLSearchParams)) {" +
                        ...
                        + ".convertBodyToArrayBuffer(body).then(function(arrayBuffer) {" +
                        "            return Array.from(new Uint8Array(arrayBuffer));" +
                        "        });" +
                        "    }," +
        ...
        public static final String PLATFORM_READY_JS_SOURCE = "(function() {" +
                        "  if ((window.top == null || window.top === window) && window." + JAVASCRIPT_BRIDGE_NAME
                        + " != null && window." + JAVASCRIPT_BRIDGE_NAME + "._platformReady == null) {" +
